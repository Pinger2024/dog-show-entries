/* AUTHORITATIVE DESIGN SOURCE — verbatim from claude.ai/design project "Remi",
   file remi/green-journey.jsx. Every literal value here is the fidelity
   contract. See DEVIATIONS.md for the short list of agreed differences. */
/* global React, I, Phone, HomeBar, Browser, ClubLogo */
// ════════════════════════════════════════════════════════════
// GREEN JOURNEY — the rest of the finalised flow, same skin as
// green-live: Browse → (Show page = GreenLive.Page) → Enter →
// Confirmed, plus a rebuilt Desktop show page. Friendly font,
// livelier green, entry numbers private, real eligibility engine.
// ════════════════════════════════════════════════════════════
(function () {
  const G = window.GreenLive.tokens;
  const K = window.GreenLive.kit;
  const FSETS = window.GreenLive.FSETS;
  const { H, BODY, Eyebrow, Wordmark, Btn, Chip, Card, SecLabel, Pulse, Countdown } = K;
  const money = (v) => "£" + v;
  const R = () => window.REMI;
  const SC = () => window.REMI.showcase;
  const vars = (fontset) => {
    const fs = FSETS[fontset || "friendly"];
    return { ["--gl-head"]: fs.head, ["--gl-body"]: fs.body, ["--gl-hw"]: fs.hw, ["--gl-tight"]: fs.tight, fontFamily: fs.body };
  };
  ...
  function Browse(opts) {
    return (
      <Phone bg={G.paper} statusColor={G.ink}>
        <div style={{ overflow: "hidden", color: G.ink, ...vars(opts && opts.fontset) }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 18px 10px" }}>
            <Wordmark size={20} />
            <div style={{ display: "flex", gap: 14, color: G.ink2 }}>{React.cloneElement(I.search, { size: 20 })}{React.cloneElement(I.user, { size: 20 })}</div>
          </div>
          ...
          <div style={{ padding: "20px 18px 30px", textAlign: "center" }}>
            <div style={{ ...BODY, fontSize: 11.5, color: G.ink3 }}>Powered by</div>
            <div style={{ marginTop: 3 }}><Wordmark /></div>
          </div>
        </div>
      </Phone>
    );
  ...
  function Enter(opts) {
    ...
    return (
      <Phone bg={G.paper} statusColor={G.ink}>
        <div style={{ display: "flex", flexDirection: "column", height: "100%", color: G.ink, ...vars(opts && opts.fontset) }}>
  ...
  function Confirmed(opts) {
    ...
    return (
      <Phone bg={G.paper} statusColor={G.cream}>
        <div style={{ overflow: "hidden", color: G.ink, background: G.deep, ...vars(opts && opts.fontset) }}>
          ...
          <div style={{ background: G.paper, borderTopLeftRadius: 22, borderTopRightRadius: 22, marginTop: -2, padding: "18px 16px 28px", position: "relative" }}>
            ...
            <Card style={{ padding: "6px 18px" }}>
              {[["Now", "Entry confirmation lands in your inbox"], [sc.closed.runningOrder, "Running order & your ring emailed to you"], [s.dateFull.split(" ").slice(0, 2).join(" "), `Doors ${s.doors} — bring your confirmation`]].map(([k, t], i) => (
            ...
            <Card style={{ padding: 16, marginTop: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 42, height: 42, borderRadius: 11, background: G.freshSoft, color: G.freshDeep, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{React.cloneElement(I.document, { size: 19 })}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ ...H, fontWeight: 600, fontSize: 16 }}>Add a catalogue · {money(s.fees.catalogue)}</div>
                  <div style={{ ...BODY, fontSize: 12, color: G.ink3 }}>Collected with your ring number on the day.</div>
                </div>
                <Btn kind="primary" sm>Add</Btn>
              </div>
            </Card>
  ...
  function Desktop(opts) {
    ...
    return (
      <Browser url={R().shareUrl} bg={G.paper} barBg={G.paper2} barText={G.ink3}>
        <div style={{ height: "100%", overflow: "hidden", color: G.ink, ...vars(opts && opts.fontset) }}>
          {/* nav */}
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 44px", borderBottom: `1px solid ${G.line}`, background: G.surface }}>
            <Wordmark size={20} />
            <div style={{ display: "flex", gap: 26, ...BODY, fontSize: 13.5, color: G.ink2 }}><span style={{ color: G.ink, fontWeight: 600 }}>Shows</span><span>For clubs</span><span>Help</span></div>
            <Btn kind="ghost" sm>{React.cloneElement(I.user, { size: 15 })} Sign in</Btn>
          </div>
          {/* photo banner */}
          <div style={{ position: "relative", height: 260, background: `linear-gradient(120deg, ${G.deep}, ${G.deepest})` }}>
            <div style={{ position: "absolute", inset: 0 }}>
              <image-slot id="cvDeskBanner" shape="rect" fit="cover" placeholder="Drop a venue banner photo" style={{ width: "100%", height: "100%" }}></image-slot>
            </div>
            ...
            <Chip tone="onDark" style={{ position: "absolute", top: 16, right: 44, backdropFilter: "blur(6px)" }}>{React.cloneElement(I.camera, { size: 13 })} Your photo here</Chip>
          </div>
          {/* body */}
          <div style={{ display: "flex", gap: 40, padding: "34px 44px", alignItems: "flex-start" }}>
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ marginTop: 32, maxWidth: 680 }}>
                ...
                <Card style={{ padding: "16px 20px 18px" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 14, paddingTop: 13, borderTop: `1px solid ${G.line}`, ...BODY, fontSize: 12, color: G.ink3, lineHeight: 1.45 }}>
                    {React.cloneElement(I.info, { size: 14, style: { flexShrink: 0, marginTop: 1 } })} {cf.variety}
                  </div>
                </Card>
              </div>
              {/* at the show */}
              <div style={{ marginTop: 32, maxWidth: 680 }}>
                ...
                <Card style={{ padding: "8px 20px 16px" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", columnGap: 18 }}>
                    {[[I.shield, "Unbenched"], [I.sun, "Covered exercise"], [I.car, "Free parking"], [I.mail, "Postal entries"], [I.shield, "Vet on call"], [I.rosette, "NFC welcome"]].map(([ic, t], i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: 9, padding: "9px 0", ...BODY, fontSize: 13.5, color: G.ink2 }}><span style={{ color: G.freshDeep, display: "flex" }}>{React.cloneElement(ic, { size: 16 })}</span>{t}</div>
                    ))}
                  </div>
            ...
            <div style={{ width: 344, flexShrink: 0, display: "flex", flexDirection: "column", gap: 16 }}>
              <Card style={{ overflow: "hidden" }}>
                ...
                <div style={{ padding: 16 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {[["Date", `${s.dayShort} ${s.day} ${s.monthShort}`], ["Judging", s.judging], ["Venue", s.venue.name], ["Best in Show", s.bis]].map(([k, v]) => (
              ...
              <Card style={{ padding: 16, display: "flex", gap: 12, alignItems: "center" }}>
                <div style={{ width: 74, height: 74, borderRadius: 10, background: "#fff", border: `1px solid ${G.line2}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, color: G.ink }}>{React.cloneElement(I.qr, { size: 52 })}</div>
                <div>
                  <div style={{ ...H, fontWeight: 600, fontSize: 15 }}>Share this show</div>
                  <div style={{ ...BODY, fontSize: 12.5, color: G.ink3, marginTop: 2, lineHeight: 1.4 }}>Scan for ringside, or send the link — it unfurls into a card.</div>
                  <div style={{ display: "flex", gap: 6, marginTop: 8 }}><Chip tone="fresh" style={{ height: 27 }}>{React.cloneElement(I.whatsapp, { size: 13 })} WhatsApp</Chip><Chip style={{ height: 27 }}>Copy</Chip></div>
                </div>
              </Card>
            </div>
          </div>
