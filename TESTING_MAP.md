...
| 53 | Edit schedule data (sponsors, judge bios, etc.) | `secretary.updateScheduleData` | 🟡 | ✅ | `secretary-schedule-judges.test.ts` — saves scheduleData JSONB + show-level fields (showOpenTime, judgingStartTime, onCallVet); syncs new officers + guarantors into organisationPeople; case-insensitive dedup. `schedule-complete-signal.test.ts` — venue name/address/postcode resolves to `shows.venueId` (create / reuse-by-name / rename-in-place / blank=no-op) + `getChecklistAutoDetect.schedule_complete` (venue+times, no false-positive on bare scheduleData); beacon venue parity in `schedule-autosave-route.test.ts` |
...
| # | Journey | Procedures / Routes | Pri | Status | Notes |
|---|---|---|---|---|---|
| 126 | Entry confirmation email | `sendEntryConfirmationEmail` | 🟡 | ✅ | Call-wiring in `stripe-webhook.test.ts`; payload (to/subject/from/replyTo + dog & class in body) asserted in `entry-confirmation-email.test.ts` via `vi.importActual` (real sender → mocked Resend SDK) |
| 127 | Secretary new-entry notification | `sendSecretaryNotificationEmail` | 🟡 | 🟠 | Call-wiring asserted in `stripe-webhook.test.ts` (`toHaveBeenCalledWith(order.id)`); payload assertable via the #126 `importActual` pattern when prioritised |
