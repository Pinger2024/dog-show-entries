...
export const PRINT_PRODUCTS: PrintProduct[] = [
  ...
  },
  {
    ...
    label: 'Prize Cards',
    // A3 4-up model: 4 placement designs on one A3 sheet, secretary guillotines.
    // 25 sheets = 100 cards. Artwork generated by the sharp compositor, uploaded
    // to R2, submitted to Mixam as a single FLYERS order.
    description: 'A3 silk card, 4 designs per sheet — guillotine to A5. 25 sheets = 100 cards.',
    tradeprintProductName: 'Flyers',
    tradeprintProductId: 'PRD-CMHPUMSF',
    defaultSpecs: {
      'Size': 'A3 Landscape',
      'Paper Type': '300gsm Art Paper Silk Finish',
      'Lamination': 'None',
    },
    presets: [
      {
        id: 'standard',
        label: '350gsm Silk',
        description: 'A3 landscape, 350gsm silk — premium card stock, guillotine to A5 at home',
        tier: 'standard',
        specs: {
          'Size': 'A3 Landscape',
          'Paper Type': '300gsm Art Paper Silk Finish',
    ...
    configurableSpecs: [],
    // Quantity = number of A3 sheets. 4 cards per sheet. Mixam minimum is 25.
    suggestQuantity: (stats) =>
      Math.max(25, Math.ceil((stats.totalClasses * stats.placementsPerClass) / 4)),
  },
  {
...
export function specsToMixamItemSpecification(
  ...
  switch (product.documentType) {
    ...
    case 'prize_cards': {
      // Prize cards — A3 4-up FLYERS. Fixed A3 landscape, 300gsm silk
      // (closest Mixam API weight to the 350gsm we spec manually).
      // Artwork is a single JPEG with all 4 placement designs tiled.
      return {
        copies: quantity,
        product: 'FLYERS',
        components: [
          {
            componentType: 'FLAT',
            format: MIXAM_FORMAT.A3,
            standardSize: 'NONE',
            orientation: 'LANDSCAPE' as const,
            colours: 'PROCESS',
            substrate: {
              typeId: MIXAM_SUBSTRATE_TYPE.SILK,
              weightId: MIXAM_SUBSTRATE_WEIGHT['300gsm'],
            },
          },
        ],
      };
