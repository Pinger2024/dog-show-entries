...
export function BestAwardsPage({ show }: FrontMatterProps) {
  ...
  return (
    <Page size="A5" style={styles.frontMatterPage} wrap>
      ...
      {/* Jurisdiction block used to live here but was orphaning
          onto a near-empty page on shows with long Best Awards
          lists. Now rendered at the end of the exhibitor index
          page (catalogue-ringside.tsx / catalogue-by-class.tsx)
          where it fills trailing whitespace. */}
...
export function ExhibitorIndexPage({ show, entries, breedName }: ExhibitorIndexPageProps) {
  ...
  return (
    <Page size="A5" style={styles.frontMatterPage} wrap>
      ...
      {/* RKC-required jurisdiction statement, rendered at the very
          end so it fills trailing whitespace rather than forcing
          its own page in the front matter. */}
      <JurisdictionBlock />
