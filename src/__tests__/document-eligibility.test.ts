...
// regionals only offer SV documents (the "By Class" catalogue, ring numbers,
// schedule, and their own SV Graded Results report). See documents-not-
...
const ALWAYS_VISIBLE_KEYS = ['catalogue-by-class', 'ring-numbers', 'schedule'] as const;

describe('documentRowVisible', () => {
  it('hides all 7 RKC-only rows on a wusv show', () => {
    for (const key of RKC_ONLY_KEYS) {
      expect(documentRowVisible(key, { showRuleset: 'wusv', showType: 'open' }), key).toBe(false);
    }
  });

  it('keeps by-class catalogue, ring numbers, and schedule visible on a wusv show', () => {
  ...
  it('shows all 7 RKC-only rows on an rkc show', () => {
    for (const key of RKC_ONLY_KEYS) {
